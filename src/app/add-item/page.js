...
export default function AddItem() {
  ...
  const [widen, setWiden] = useState(false); // checkbox choice: India-only vs broaden
...
useEffect(() => {
  ...
  const openScanner = async () => {
    ...
    try {
      ...
      Object.assign(closeBtn.style, { position: 'absolute', top: 8, right: 8, padding: '6px 10px', borderRadius: 6, background: '#fff', border: 'none', cursor: 'pointer' });
      closeBtn.onclick = () => {
        cleanup();
      };
      ...
      cleanup();
      await fetchFoods('', scannedCode); // barcode lookup
      // if a single item returned and user likely wants it, auto-pick
      // (we don't auto-save — user must press Use/Save)
    } catch (err) {
  ...
  /* -------- Actions -------- */
  const pickResult = (p) => {
    // autofill form fields and expiry automatically (per your choice)
    ...
    // Autofil expiry ALWAYS as requested
  ...
  const macroSummary = () => {
    ...
    return {
      ...
      total: multiplier ? { kcal: +(kcal100 * multiplier).toFixed(0), p: +(p100 * multiplier).toFixed(1), c: +(c100 * multiplier).toFixed(1), f: +(f100 * multiplier).toFixed(1) } : null
    };
  };
